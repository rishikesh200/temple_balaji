...
const PoojaBookingsPage = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        setLoading(true);
        const result = await poojaAPI.getBookings(token, filters);
        if (result.success) {
  ...
  const handleSendConfirmation = async (bookingId) => {
    try {
      const result = await poojaAPI.sendConfirmation(token, bookingId, {
        method: 'whatsapp',
      });
  ...
  const handleSendReminder = async (bookingId) => {
    try {
      const result = await poojaAPI.sendReminder(token, bookingId);
  ...
  return (
    <AdminLayout>
      <h2 className="text-3xl font-bold mb-6">Pooja Bookings</h2>
          ...
          <table className="w-full">
            <thead className="bg-gray-100 border-b">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-sm font-semibold">Pooja Type</th>
            ...
            <tbody>
              {bookings.map((booking) => (
                <tr key={booking._id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="px-6 py-3 text-sm">{booking.poojaType}</td>
                  <td className="px-6 py-3 text-sm">
